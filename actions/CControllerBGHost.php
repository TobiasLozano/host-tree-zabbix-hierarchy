...
abstract class CControllerBGHost extends CController {
		...
		foreach ($host_groups as $group_name => $group_data) {
			if ($group_data['parent_group_name'] == '' && $group_data['children']) {
				foreach ($group_data['children'] as $subgroup_name) {
					...
					$this->group_add_problems_from_child($host_groups, $group_name, $subgroup_name);
				}
			}
		}
	...
	protected function get_hosts_for_group(&$groups, $group_name, $group_data, $filter) {
		...
		// First, get hosts from child groups that are not collapsed
			...
			foreach ($group_data['children'] as $g_name) {
				if (array_key_exists($g_name, $groups)) { // The child group was not removed from "visible" on > 1 pages
					if ($groups[$g_name]['is_collapsed']) // The group is collapsed
						continue;
					$hosts_of_sub_group = array_merge($hosts_of_sub_group, $this->get_hosts_for_group($groups, $g_name, $groups[$g_name], $filter));
				}
			}
		}
		
		// Then, get hosts directly in this group
		if (!array_key_exists('hosts', $group_data)) {
			return $hosts_of_sub_group;
		}
	...
	protected function group_add_problems_from_child(&$host_groups, $group_name, $subgroup_name) {
		if ($host_groups[$subgroup_name]['children']) {
			foreach ($host_groups[$subgroup_name]['children'] as $sub_subgroup_name) {
				$this->group_add_problems_from_child($host_groups, $subgroup_name, $sub_subgroup_name);
			}
		}
		// Add problems from child group to parent
		for ($severity = TRIGGER_SEVERITY_COUNT - 1; $severity >= TRIGGER_SEVERITY_NOT_CLASSIFIED; $severity--) {
			$host_groups[$group_name]['problem_count'][$severity] += $host_groups[$subgroup_name]['problem_count'][$severity];
		}
	}

	/**
	 * Adds parent group and all ancestor groups recursively
	 *
	...
	protected function add_parent(&$host_groups, $host_groups_dict, &$fake_group_id, $groupname_full, $filter, $expanded_groups) {
		...
		if (array_key_exists($parent_group_name, $host_groups)) {
			...
			// Recursively add parents of the parent if they don't exist (supports unlimited nesting)
			$parent_grp_arr = explode('/', $parent_group_name);
			if (count($parent_grp_arr) > 1) {
				$this->add_parent($host_groups, $host_groups_dict, $fake_group_id, $parent_group_name, $filter, $expanded_groups);
			}
		}
